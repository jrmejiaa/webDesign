        ...
        if (fileName === "index.php" || fileName === "") {
            ...
            function computeShopping(event) {
                ...
                if (gift.value == "") {
                    ...
                    if (ticketsDay > 0) {
                        productList.push(
                            `Pase por día: \t<span>${ticketsDay}</span>`
                        );
                    }
                    if (ticket2Days > 0) {
                        productList.push(
                            `Pase por 2 días: \t<span>${ticket2Days}</span>`
                        );
                    }
                    if (ticketAllDay > 0) {
                        productList.push(
                            `Pase por todo la Conferencia: \t<span>${ticketAllDay}</span>`
                        );
                    }
                    if (countTShirt > 0) {
                        productList.push(
                            `Tus Camisetas: \t<span>${countTShirt}</span>`
                        );
                    }
                    if (countEtiquetas > 0) {
                        productList.push(
                            `Tus Etiquetas: \t<span>${countEtiquetas}</span>`
                        );
                    }
                    // Display ON or OFF
            ...
            function showCalendar() {
                ...
                for (let i = 0; i < dayElect.length; i++) {
                    document.getElementById(dayElect[i]).style.display =
                        "block";
                }
            }
...
$(function () {
    // JQuery simple form to wait until the HTML is finished
    "use strict";
    // It uses to run the JavaScript after all the HTML code has deployed
    ...
    $(".summaryEvent li:nth-child(1) p:first").animateNumber(
        { number: 6 },
        1200
    );
    $(".summaryEvent li:nth-child(2) p:first").animateNumber(
        { number: 15 },
        1500
    );
    $(".summaryEvent li:nth-child(3) p:first").animateNumber(
        { number: 3 },
        1200
    );
    $(".summaryEvent li:nth-child(4) p:first").animateNumber(
        { number: 9 },
        1500
    );
